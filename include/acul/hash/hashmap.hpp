...
namespace acul
{
    ...
    public:
        ...
        public:
            ...
            basic_iterator(const hashmap *h, size_type idx) noexcept : _h(h), _idx(idx) {}
            template <typename P>
            basic_iterator(const basic_iterator<P> &other) noexcept : _h(other._h), _idx(other._idx)
            {
            }

            reference operator*() const noexcept { return _h->_pairs[_idx]; }
            pointer operator->() const noexcept { return &_h->_pairs[_idx]; }
        ...
        hashmap(size_type bucket_count = 8) noexcept
            : _pairs(nullptr), _next(nullptr), _mask(0), _num_buckets(0), _num_filled(0), _last(0)
        {
            rehash(bucket_count);
        }
        ...
        hashmap(const hashmap &rhs)
        {
            ...
            for (size_type i = 0; i < _num_buckets; ++i)
                if (_next[i] != AHM_INACTIVE) ::new ((void *)&_pairs[i]) value_type(rhs._pairs[i]);
        ...
        hashmap &operator=(const hashmap &rhs)
        {
            ...
            if (!rhs._pairs)
            {
                ...
                {
                    clear();
                    Allocator::deallocate((raw_pointer)_pairs);
                }
                _pairs = nullptr;
                _next = nullptr;
                _num_buckets = _mask = _num_filled = _last = 0;
            ...
            for (size_type i = 0; i < rhs._num_buckets; ++i)
                if (tmp._next[i] != AHM_INACTIVE) ::new ((void *)&tmp._pairs[i]) value_type(rhs._pairs[i]);
        ...
        hashmap(InputIt first, InputIt last, size_type bucket_count = 8)
            : _pairs(nullptr), _next(nullptr), _mask(0), _num_buckets(0), _num_filled(0), _last(0)
        {
        ...
        ~hashmap()
        {
            ...
            if constexpr (!std::is_trivially_destructible_v<value_type>)
                for (size_type i = 0; i < _num_buckets; ++i)
                    if (_next[i] != AHM_INACTIVE) _pairs[i].~value_type();
        ...
        void rehash(size_type required)
        {
            ...
            const size_t total = bytes_pairs + bytes_next;

            auto *base = Allocator::allocate(total);
            auto *npairs = reinterpret_cast<value_type *>(base);
            auto *nnext = reinterpret_cast<uint32_t *>((raw_pointer)(base) + bytes_pairs);
            ...
            if (_pairs)
            {
                ...
                rehash_reserve_heads(_pairs, _next, _num_buckets, v);
                rehash_move_pass(_pairs, _next, _num_buckets, v);
        ...
        inline iterator find(const K &key) noexcept { return iterator(this, bucket(key)); }
        inline const_iterator find(const K &key) const noexcept { return const_iterator(this, bucket(key)); }
        ...
        size_type erase(const K &key) noexcept
        {
            ...
            if (_eq(_pairs[b0].first, key))
            {
                ...
                if constexpr (!std::is_trivially_destructible_v<value_type>) _pairs[b0].~value_type();
                ::new ((void *)&_pairs[b0]) value_type(std::move(_pairs[nb]));
        ...
        iterator erase(const_iterator pos) noexcept
        {
            ...
            const size_type kmain = key_to_bucket(_pairs[i].first);
            ...
            if (i == kmain)
            {
                ...
                ::new ((void *)&_pairs[i]) value_type(std::move(_pairs[nb]));
                _next[i] = (nx == nb) ? i : nx;
                clear_bucket(nb);
                --_num_filled;
        ...
        void swap(hashmap &a, hashmap &b) noexcept
        {
            using std::swap;
            swap(a._pairs, b._pairs);
        ...
        std::optional<value_type> extract(const key_type &key) noexcept
        {
            ...
            if (_eq(_pairs[b0].first, key))
            {
                std::optional<value_type> out(std::move(_pairs[b0]));
            ...
            for (;;)
            {
                if (_eq(_pairs[cur].first, key))
                {
                    std::optional<value_type> out(std::move(_pairs[cur]));
                    const size_type nx = _next[cur];
                    _next[prev] = (nx == cur) ? prev : nx;
                    clear_bucket(cur);
                    --_num_filled;
    ...
    private:
        ...
        static constexpr H _hasher = H{};
        static constexpr Eq _eq = Eq{};

        void allocate_blocks(size_type bucket_count)
        {
            if (bucket_count < 8) bucket_count = 8;
            const size_t bytes_pairs = size_t(bucket_count) * sizeof(value_type);
            const size_t bytes_next = size_t(bucket_count) * sizeof(uint32_t);
            const size_t total = bytes_pairs + bytes_next;

            void *base = Allocator::allocate(total);
            _pairs = reinterpret_cast<value_type *>(base);
            _next = reinterpret_cast<uint32_t *>(static_cast<uint8_t *>(base) + bytes_pairs);
                                                   ...
                                                   rehash_view &v) noexcept
        {
            for (size_type i = 0; i < old_b; ++i)
            {
                ...
                const size_type b0 = size_type(_hasher(opairs[i].first) & v.nmask);
                v.aux.reserved[b0] = 1;
            }
        }
                                               ...
                                               rehash_view &v) noexcept
        {
            for (size_type i = 0; i < old_b; ++i)
            {
                ...
                const size_type b0 = size_type(_hasher(e.first) & v.nmask);
        ...
        {
            ::new ((void *)&_pairs[pos]) value_type(std::forward<Kx>(k), std::forward<Vx>(v));
        ...
        {
            const auto sel = find_slot(kv.first);
            if (sel.existed) return {iterator(this, sel.pos), false};
            return place_kv_at(sel.pos, sel.link_from, kv.first, kv.second);
        }
        ...
        ACUL_FORCEINLINE pair<iterator, bool> insert_kv(value_type &&kv)
        {
            const auto sel = find_slot(kv.first);
            if (sel.existed) return {iterator(this, sel.pos), false};
            return place_kv_at(sel.pos, sel.link_from, std::move(kv.first), std::move(kv.second));
        }
        ...
        inline size_type kickout_bucket(size_type kmain, size_type kbucket) noexcept
        {
            ...
            ::new (_pairs + new_bucket) value_type(std::move(_pairs[kbucket]));
        ...
        alloc_res find_slot(const K &key) noexcept
        {
            const size_type b0 = key_to_bucket(key);
            const size_type n0 = _next[b0];

            if (n0 == AHM_INACTIVE) return {b0, AHM_INACTIVE, false};
            if (_eq(_pairs[b0].first, key)) return {b0, AHM_INACTIVE, true};

            const size_type kmain = key_to_bucket(_pairs[b0].first);
            if (kmain != b0)
            {
                if (_next[kmain] == AHM_INACTIVE)
                {
                    const size_type old_next = _next[b0];
                    ::new ((void *)&_pairs[kmain]) value_type(std::move(_pairs[b0]));
            ...
            for (;;)
            {
                if (_eq(_pairs[cur].first, key)) return {cur, AHM_INACTIVE, true};
                const size_type nx = _next[cur];
                if (nx == cur)
                {
        ...
        ACUL_FORCEINLINE size_type key_to_bucket(const K &k) const noexcept { return size_type(_hasher(k) & _mask); }
