...
class StringPool
{
    static_assert(is_string_char_v<T>, "StringPool requires a string character type");
...
public:
    ...
    explicit StringPool(size_type poolSize) : _poolSize(poolSize), _data(allocator.allocate(poolSize)), _pos(0) {}

    ~StringPool() noexcept { allocator.deallocate(_data, _poolSize); }

    StringPool(const StringPool &other) noexcept
    ...
    StringPool &operator=(const StringPool &other) noexcept
    {
        if (this != &other)
        {
...
private:
    size_type _poolSize;
    T *_data;
    DArray<size_type> _lines;
    size_type _pos;
};

template <typename T, template <typename> class Allocator>
typename StringPool<T, Allocator>::ArrayAllocator StringPool<T, Allocator>::allocator{};

template <typename T, template <typename> class Allocator>
class StringPool<T, Allocator>::Iterator
{
public:
    ...
    Iterator(DArray<size_t> &lines, pointer ptr = nullptr, size_t pos = 0) : _ptr(ptr), _lines(lines), _pos(pos) {}
