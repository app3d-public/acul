...
namespace acul
{
    ...
    path path::operator/(const path &other) const
    {
        ...
        {
            if (other._nodes[start_idx] == ".." && !p._nodes.empty()) p._nodes.pop_back();
            else break;
        }
    ...
    void path::parse_relative_part(const char *&start, const char *end)
    {
        ...
        while (const char *sep = get_separator(start, end))
        {
            ...
            bool is_sub = name == "..";
            if (is_sub && was_added) _nodes.pop_back();
            else if (name != ".")
            {
    ...
    size_t path::build_scheme_part(string &result, char sep) const
    {
        ...
        if (_flags & FLAG_ABS)
        {
            ...
            {
                if (_scheme == "unc") result += "//";
                else if (_scheme != "file") result += _scheme + "://";
                else result += sep;
            }
        }
        return start;
